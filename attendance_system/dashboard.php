...
// กำหนดค่า $user_id และ $role จาก Session
...
// ดึงข้อมูลเวรจากตาราง duties
if ($role === 'admin') {
    $duty_sql = "SELECT title, date FROM duties"; // ดึงข้อมูลทั้งหมดสำหรับ admin
} else {
    $duty_sql = "SELECT title, date FROM duties WHERE user_id = $user_id"; // ดึงข้อมูลเฉพาะของ user
}

$duty_result = $conn->query($duty_sql);

if (!$duty_result) {
    die("เกิดข้อผิดพลาดในการดึงข้อมูลเวร: " . $conn->error);
}

// เพิ่มข้อมูลเวรเข้าไปใน $calendar_events
$calendar_events = []; // เริ่มต้น $calendar_events เป็น array ว่าง
while ($row = $duty_result->fetch_assoc()) {
    $calendar_events[] = [
        'title' => $row['title'], // ชื่อเวร
        'start' => $row['date'],  // วันที่เริ่มต้น
        'color' => '#007bff'      // สีฟ้า
    ];
}
...
<div class="modal" id="leaveModal" tabindex="-1" aria-labelledby="leaveModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">    <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addDutyModal">
        <i class="bi bi-calendar-plus"></i> เพิ่มเวร
    </button>
...
<div class="sidebar" id="sidebar">
  ...
  <a href="LeaveRequests.php" class="menu-item"><i class="bi bi-calendar-check"></i><span class="menu-text">รายงานการลา</span></a>
  <!-- <a href="cert.php" class="menu-item"><i class="bi bi-file-earmark-text"></i><span class="menu-text">หนังสือรับรอง</span></a> -->
  <!-- <a href="notify.php" class="menu-item"><i class="bi bi-bell"></i><span class="menu-text">แจ้งเตือน</span></a> -->
  <a href="news.php" class="menu-item"><i class="bi bi-megaphone"></i><span class="menu-text">ข่าวสาร</span></a>
...
<div class="container my-5">
    ...
    <div id="calendar"></div>
    <?php if ($role === 'admin'): ?>
    <!-- ปุ่มสำหรับเปิด Modal เพิ่มเวร -->
    <button class="btn btn-success mt-3" data-bs-toggle="modal" data-bs-target="#addDutyModal">
        <i class="bi bi-calendar-plus"></i> เพิ่มเวร
    </button>
<?php endif; ?>
</div>
...
<!-- Modal สำหรับเพิ่มเวร -->
<div class="modal fade" id="addDutyModal" tabindex="-1" aria-labelledby="addDutyModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addDutyModalLabel">เพิ่มเวร</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="addDutyForm" action="add_duty.php" method="POST">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="dutyTitle" class="form-label">ชื่อเวร</label>
                        <input type="text" id="dutyTitle" name="title" class="form-control" required>
                    </div>
                    <div class="mb-3">
                        <label for="dutyDate" class="form-label">วันที่</label>
                        <input type="date" id="dutyDate" name="date" class="form-control" required>
                    </div>
                    <button type="submit" class="btn btn-primary">เพิ่มเวร</button>
                </form>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    console.log(<?= json_encode($calendar_events) ?>);
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: <?= json_encode($calendar_events) ?> // ส่งข้อมูล JSON ไปยัง FullCalendar
    });
    calendar.render();
});
</script>
